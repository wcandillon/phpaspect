...
<!--
Generates xslt expressions from Before advices
...
<xsl:template match="php:*">
    <xsl:if test="name(.) = 'php:start' and not(descendant::php:start)">
        <xsl:for-each select="//php:xpath_expr[preceding-sibling::php:T_BEFORE]">
                <xsl:element name="xsl:if" namespace="&xsl;">
                    ...
                    -->
                    <xsl:element name="php:T_FUNCTION" namespace="&php;">function</xsl:element>
