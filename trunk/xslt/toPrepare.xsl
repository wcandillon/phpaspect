...
<xsl:template match="php:*">
    <xsl:choose>
        <xsl:when test="php:function('XPathFunctions::hasId', @id)">
            <xsl:element name="php:T_VARIABLE" namespace="&php;">$phpaspect_<xsl:value-of select="php:function('XPathFunctions::getId', @id)" /></xsl:element>
        </xsl:when>
        <xsl:otherwise>
            <xsl:copy>
